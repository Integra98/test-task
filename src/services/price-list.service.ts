...
export class PriceListService {
  ...
  getTestPriceList(SearchTerm?: string) {
    if(SearchTerm) {
          return this.priceLists.filter(price => {
      return price.priceListID?.toString().includes(SearchTerm)
        || price.extErpPriceListID?.toString().includes(SearchTerm)
        || price.priceListName?.toString().toLowerCase().includes(SearchTerm);
    });
    } else
